...
def mengubah_data(mahasiswa):
    apa_diubah = input("Apa yang ingin diubah \nNama/Nilai: \n")
...
while pilihan != 5:
    ...
    if pilihan == 1:
        mahasiswa = membuat_objek()
    elif pilihan == 2:
        tampilkan_objek(mahasiswa)
    elif pilihan == 3:
        if mahasiswa is not None:
            mengubah_data(mahasiswa)
        else:
            print("Objek belum dibuat")
    elif pilihan == 4:
        if mahasiswa is not None:
        ...
        print("Tidak valid. Silakan pilih (1/2/3/4/5)")